(function() {
	...
	angular
		...
		function MainController($http, $scope, $sce, province, config) {
			...
			function onSubmit() {
				...
				var token="";
				console.log(config.url.repomanager.version)
				$http({
					method: 'GET',
				...
				operationData.opClass = config.opClass;
				operationData.domainid = config.domainId;
				var ds = (new Date()).toISOString().replace(/[^0-9]/g, "");

				operationData.operationid = "XX" + ds;

				var files =[];
				var staticDocNames ="[{\"block\":1,\"code\":\"" + config.templateName + "\",\"description\":\"Plantilla\",\"mandatory\":1,\"name\":\"Plantilla Hazte Cliente\",\"order\":1,\"signmultiple\":0,\"signorupload\":0,\"transferible\":0,\"uploadMandatory\":1,\"visiblesign\":1}]";
				...
				$scope.loading = true;
				console.log(config.url.repomanager.login)
				$http({
					method: 'POST',
					...
					console.log(config.url.repomanager.operations.newoperationmultilevel)
					$http({
						method: 'POST',
						...
						console.log(uuid);
						console.log(config.url.esign.signop);
